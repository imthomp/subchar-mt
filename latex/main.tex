...
\begin{abstract}
A persistent hypothesis in Chinese NLP holds that decomposing characters into sub-character units---radicals, morphemes, or phonetic features---should improve translation by exposing semantic structure hidden inside logographic script. We test this hypothesis across nine tokenization schemes, two pretrained MT models, and training sets of 50--1000 sentence pairs, evaluating with both corpus BLEU and COMET. We find that any representation increasing token count inflates corpus BLEU through a length-based artifact while consistently hurting COMET; the raw-character baseline achieves the best COMET in all 20 model$\times$size conditions. Probing classifiers on encoder hidden states reveal the mechanism: radical decomposition destroys the frequency and part-of-speech encodings present in pretrained representations, while morpheme segmentation preserves syntax but misaligns with pretrained subword boundaries. Neither selective decomposition, semantic gloss injection, nor a Zh$\to$Ja cross-lingual extension recovers baseline quality. LLM-as-judge evaluation with three open models corroborates these findings. Our results suggest that sub-character gains in Chinese MT are largely an evaluation artifact, and that raw characters already provide an effective surface for low-resource LoRA fine-tuning of pretrained MT models.
\end{abstract}
...
\item We confirm the main finding under LLM-as-judge evaluation \citep{dubey-etal-2024-llama3,ustun-etal-2024-aya,qwen-team-2025-qwen25} using three open models (Llama~3.1~8B, Aya~Expanse~8B, Qwen2.5-32B): the strongest judge (Qwen2.5-32B) prefers the baseline in all conditions, with morphemes receiving only 3/100 wins on opus-mt. Neither MT model in this study uses Qwen architecture; both are transformer encoder-decoders trained specifically for translation (opus-mt, NLLB-200).
\end{enumerate}
